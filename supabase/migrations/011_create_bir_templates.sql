...
CREATE TABLE bir_form_templates (
...
CREATE UNIQUE INDEX idx_bir_templates_form_current ON bir_form_templates(form_number) WHERE is_current = true;
...
CREATE TABLE bir_form_field_mappings (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_bir_field_map_template ON bir_form_field_mappings(template_id, display_order);
...
CREATE TABLE bir_tax_form_records (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_bir_records_client ON bir_tax_form_records(client_id, form_number, filing_period);
CREATE INDEX idx_bir_records_status ON bir_tax_form_records(status) WHERE status != 'exported';
